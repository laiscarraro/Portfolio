...
!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(16)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (audiocontext) {
  /**
  ...
  var Master = function Master() {
    ...
    this.output = audiocontext.createGain(); //put a hard limiter on the output
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  p5.prototype.soundFormats = function () {
    ...
    p5sound.extensions = []; // add extensions
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  p5.Filter = function (type) {
    Effect.call(this); //add extend Effect by adding a Biquad Filter
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  p5.Oscillator = function (freq, type) {
    ...
    this.oscillator.frequency.setValueAtTime(this.f, p5sound.audiocontext.currentTime); // connections
  ...
  p5.Oscillator.prototype.freq = function (val, rampTime, tFromNow) {
    if (typeof val === 'number' && !isNaN(val)) {
      ...
      var rampTime = rampTime || 0;
      var tFromNow = tFromNow || 0;
      var t = now + tFromNow + rampTime; // var currentFreq = this.oscillator.frequency.value;
  ...
  p5.Oscillator.prototype.phase = function (p) {
    ...
    if (!this.dNode) {
      // create a delay node
      this.dNode = p5sound.audiocontext.createDelay(); // put the delay node in between output and panner
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  p5.MonoSynth = function () {
    ...
    this.env.setExp(true); //set params

    this.setADSR(0.02, 0.25, 0.05, 0.35); // oscillator --> env --> this.output (gain) --> p5.soundOut

    this.oscillator.disconnect();
    this.oscillator.connect(this.output);
    this.env.disconnect();
    this.env.setInput(this.output.gain); // reset oscillator gain to 1.0
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  var TimelineSignal = __webpack_require__(13);
  ...
  p5.PolySynth.prototype.noteAttack = function (_note, _velocity, secondsFromNow) {
    ...
    var secondsFromNow = ~~secondsFromNow; //this value is used by this._voicesInUse

    var acTime = p5sound.audiocontext.currentTime + secondsFromNow; //Convert note to frequency if necessary. This is because entries into this.notes
...
var moduleSources = [__webpack_require__(36)["default"]];
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  var ac = p5sound.audiocontext; // Stereo panner
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  p5.SoundFile = function (paths, onload, onerror, whileLoading) {
    ...
    this.panPosition = 0.0;
    this.panner = new p5.Panner(this.output, p5sound.input, 2); // it is possible to instantiate a soundfile with no path
  ...
  p5.SoundFile.prototype.add = function () {// TO DO
  };

  p5.SoundFile.prototype.dispose = function () {
    var now = p5sound.audiocontext.currentTime; // remove reference to soundfile
  ...
  p5.SoundFile.prototype.processPeaks = function (callback, _initThreshold, _minThreshold, _minPeaks) {
    ...
    var initialThreshold = _initThreshold || 0.9,
        ...
        minPeaks = _minPeaks || 200; // Create offline context

    var offlineContext = new window.OfflineAudioContext(1, bufLen, sampleRate); // create buffer source

    var source = offlineContext.createBufferSource();
    source.buffer = buffer; // Create filter. TO DO: allow custom setting of filter

    var filter = offlineContext.createBiquadFilter();
    filter.type = 'lowpass';
    source.connect(filter);
    filter.connect(offlineContext.destination); // start playing at time:0
    ...
    offlineContext.oncomplete = function (e) {
      ...
      var bufferData = filteredBuffer.getChannelData(0); // step 1:
      // create Peak instances, add them to array, with strength and sampleIndex

      do {
        allPeaks = getPeaksAtThreshold(bufferData, threshold);
        threshold -= 0.005;
      } while (Object.keys(allPeaks).length < minPeaks && threshold >= minThreshold); // step 2:
      // find intervals for each peak in the sampleIndex, add tempos array


      var intervalCounts = countIntervalsBetweenNearbyPeaks(allPeaks); // step 3: find top tempos

      var groups = groupNeighborsByTempo(intervalCounts, filteredBuffer.sampleRate); // sort top intervals

      var topTempos = groups.sort(function (intA, intB) {
        return intB.count - intA.count;
      }).splice(0, 5); // set this SoundFile's tempo to the top tempo ??

      this.tempo = topTempos[0].tempo; // step 4:
  ...
  function getPeaksAtThreshold(data, threshold) {
    ...
    for (var i = 0; i < length; i++) {
      if (data[i] > threshold) {
        ...
        peaksObj[i] = peak; // Skip forward ~ 1/8s to get past this peak.
    ...
    for (var index = 0; index < peaksArray.length; index++) {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        if (startPeak && endPeak) {
          ...
          var interval = endPos - startPos; // add a sample interval to the startPeak in the allPeaks array
  ...
  function getPeaksAtTopTempo(peaksObj, tempo, sampleRate, bpmVariance) {
    var peaksAtTopTempo = [];
    var peaksArray = Object.keys(peaksObj).sort(); // TO DO: filter out peaks that have the tempo and return
  ...
  function _onAudioProcess(processEvent) {
    ...
    this._lastPos = inputBuffer[inputBuffer.length - 1] || 0; // do any callbacks that have been scheduled
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  var EQFilter = __webpack_require__(57);
  /**
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = (function (require) {
  ...
  var processorNames = __webpack_require__(68);
